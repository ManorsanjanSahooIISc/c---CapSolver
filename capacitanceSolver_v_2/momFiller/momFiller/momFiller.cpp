...
void momFiller::fillMomMatrix() {
	...
	FILE *(momMat);
	momMat = fopen("cppMOM_3D_AnaOne.txt", "w");
	for (int row = 0; row < numPatches; row++) {
		...
		for (int col = 0; col < numPatches; col++) {
			...
			if (distanceCentToCent <= (10 * obsMaxLen)) {
			...
			/*else if ((distanceCentToCent > (4 * obsMaxLen)) && (distanceCentToCent <= (10 * obsMaxLen))) {
				...
				integrations.setObsPoint(obsCent);
				momEntry = constant*srcArea*integrations.integrationSevenPoint();
			}*/

			else if (distanceCentToCent>(10 * obsMaxLen)) {
				integrations.setObsPoint(obsCent);
				momEntry = constant*srcArea*integrations.integrationOnePoint();
			}
			else {
	...
	cout << "Matrix setup Time" << lutime << endl;
...
std::vector<double> momFiller::solveMom(int& i) {
	...
	fillingRhs.fillRhs(presentRhs, patchesList, numPatches, (i+1));
	//zTempMat.matrixSolve(presentRhs, ipiv);
	//zTempMat.matrixSolve(presentRhs, ipiv);
	zMat.matrixSolve(presentRhs, ipiv);
	sigma = presentRhs;
	return sigma;
	/*FILE *(cppRhs);
	cppRhs = fopen("cppRhs.txt", "w");
	for (int i = 0; i < numPatches; i++) {
		fprintf(cppRhs, "%f", presentRhs[i]);
		fprintf(cppRhs, "\n");
	}
	fclose(cppRhs);*/
}
...
void momFiller::chargeDensityCalculation(std::vector<double> sigma, int& i) {
	...
	for (int netRow = 0; netRow < numNets; netRow++) {
		sum = 0;
		for (int netCol = 0; netCol < numPatches; netCol++) {
